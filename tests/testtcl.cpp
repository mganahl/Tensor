...
int main(int argc, char** argv){
  ...
  auto t4=t1.tcldot(t2,LabelType(L1),LabelType(L2));
  if ((t4-t3).norm()>1E-10){
    cout<<"tblisdot gave wrong result!!!"<<endl;
  }
  
  cout<< "TCL: fastest of "<<Nmax<<" contractions: "<<*std::min_element(time.begin(),time.end()) << " milliseconds\n";      
  cout<< "TCL: average time per contraction: "<<std::accumulate(time.begin(),time.end(),0.0)*1.0/Nmax << " milliseconds\n";      
  return 0;
}
