...
int main(int argc, char** argv){
  ...
  uint Nmax;  
  ss1>>Nmax;
  // ShapeType s1({200,3,4,100});
  // ShapeType s2({8,200,100});
  ...
  auto t4=t1.tblisdot(t2,LabelType(L1),LabelType(L2));
  if ((t4-t3).norm()>1E-10){
    cout<<"tblisdot gave wrong result!!!"<<endl;
  }
  
  cout<< "TBLIS: fastest of "<<Nmax<<" contractions: "<<*std::min_element(time.begin(),time.end()) << " milliseconds\n";        
  cout<< "TBLIS: average time per contraction: "<<std::accumulate(time.begin(),time.end(),0.0)*1.0/Nmax << " milliseconds\n";


  //t1.gemmdot(t2,LabelType{1,2},LabelType{2,3,4});
  return 0;
}
